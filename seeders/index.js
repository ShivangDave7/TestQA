...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Nelda.Erdman38',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('N7P_ZEqyg7TPwj4') ) {
      let user = new User({
        'password':'N7P_ZEqyg7TPwj4',
        'username':'Nelda.Erdman38',
    ...
    let admin = await User.findOne({
      'username':'Carlee_Schumm',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('dkxp7LqAPhSfwQ8') ) {
      let admin = new User({
        'password':'dkxp7LqAPhSfwQ8',
        'username':'Carlee_Schumm',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('N7P_ZEqyg7TPwj4') && userRole){
    ...
    if (admin && admin.isPasswordMatch('dkxp7LqAPhSfwQ8') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
